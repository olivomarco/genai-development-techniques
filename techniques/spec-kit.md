...
Spec Kit's architecture is designed for community extensibility, with catalog infrastructure already in place (`extensions/catalog.community.json`). Community contributions include **[spec-kit-github-issues](https://github.com/Fatima367/spec-kit-github-issues)** — an extension that generates spec artifacts from GitHub Issues — an **ADIK Integration** (by mateors1), Multi-Model Review updates, and a Token Economy extension added to the community catalog. Recent releases also improved extension isolation so one failing extension does not break the whole workflow. The ecosystem is still young, but it has moved from "just starting" to visible catalog maturation.
...
**vs. Superpowers:** Both take a structured approach to AI-assisted development, but at different layers. Spec Kit focuses on the specification and planning phases — getting the requirements right before building. Superpowers focuses on the implementation methodology — teaching the agent *how* to build through composable skills (TDD, code review, debugging). Spec Kit is deliberately narrow and agent-agnostic; Superpowers is a complete development workflow supporting six specific tools. They're complementary: Spec Kit could define what to build, Superpowers could enforce how to build it.
